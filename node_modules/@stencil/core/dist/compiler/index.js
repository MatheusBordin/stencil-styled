...
const gatherVdomMeta = (m, args) => {
    ...
    if (args.length > 1) {
        if (ts$1__default.isObjectLiteralExpression(args[1])) {
...
const parseCallExpression = (m, node) => {
    if (node.arguments != null && node.arguments.length > 0) {
        if (node.expression.kind === ts$1__default.SyntaxKind.Identifier) {
            // h('tag')
            visitCallExpressionArgs(m, node.expression, node.arguments);
        }
        else if (node.expression.kind === ts$1__default.SyntaxKind.PropertyAccessExpression) {
...
const parseStaticComponentMeta = (config, compilerCtx, transformCtx, typeChecker, cmpNode, moduleFile, nodeMap, transformOpts, fileCmpNodes) => {
    ...
    const visitComponentChildNode = (node) => {
        ...
        return ts$1__default.visitEachChild(node, visitComponentChildNode, transformCtx);
    };
    ts$1__default.visitEachChild(cmpNode, visitComponentChildNode, transformCtx);
